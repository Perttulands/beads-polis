...
pub struct SqliteStorage {
    conn: Connection,
    /// Held for the lifetime of the connection; dropping releases the flock.
...
impl Drop for SqliteStorage {
    fn drop(&mut self) {
        // Explicitly unlock before the file handle is closed.
        // Default field-drop order (conn then lock_file) is also correct,
        // but an explicit unlock makes the release visible and immediate.
...
impl SqliteStorage {
    ...
    ///
    /// Acquires a POSIX advisory lock (`flock(LOCK_EX)`) on a sibling
    /// `.lock` file before opening the database.  If the lock cannot be
    /// acquired within `lock_timeout_ms` (default 30 000 ms), returns
    /// [`BeadsError::DatabaseLocked`].
    ///
    /// # Errors
    ///
    /// Returns an error if the lock times out, the connection cannot be
    /// established, or schema application fails.
    pub fn open_with_timeout(path: &Path, lock_timeout_ms: Option<u64>) -> Result<Self> {
        // Acquire an exclusive advisory lock for file-backed databases.
        let lock_file = if path.to_string_lossy() != ":memory:" {
            ...
            let timeout = Duration::from_millis(lock_timeout_ms.unwrap_or(30_000));
                ...
                match file.try_lock_exclusive() {
                    ...
                    Err(e) if e.kind() == std::io::ErrorKind::WouldBlock => {
                        if start.elapsed() >= timeout {
                            // Lock file is dropped here, no flock to release.
                            return Err(BeadsError::DatabaseLocked {
        ...
        Ok(Self { conn, lock_file })
    }

    /// Open an in-memory database for testing.
    ///
    /// No advisory lock is acquired (no file to protect).
    ///
    /// # Errors
    ///
