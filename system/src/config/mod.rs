...
";
    ...
    fn discover_beads_dir_returns_error_when_not_found() {
        ...
        // Bypass ambient BEADS_DIR from the outer test environment.
        let missing_env = temp.path().join("missing-env-beads-dir");
        let result = discover_beads_dir_with_env(Some(temp.path()), Some(&missing_env));
    ...
    fn discover_beads_dir_finds_at_root() {
        ...
        let missing_env = temp.path().join("missing-env-beads-dir");
        let discovered =
            discover_beads_dir_with_env(Some(temp.path()), Some(&missing_env)).expect("discover");
    ...
    fn discover_beads_dir_deeply_nested() {
        ...
        let missing_env = temp.path().join("missing-env-beads-dir");
        let discovered =
            discover_beads_dir_with_env(Some(&nested), Some(&missing_env)).expect("discover");
        assert_eq!(discovered, beads_dir);
    }
