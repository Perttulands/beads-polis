...
set -euo pipefail
umask 022
shopt -s lastpipe 2>/dev/null || true # REASON: lastpipe is bash 4.2+ only; not available on older bash or when piped
    ...
    case "$os" in
        ...
        linux)
            ...
            if command -v apt-get &> /dev/null; then
                (
                    sudo mkdir -p /etc/apt/keyrings 2>/dev/null # REASON: may already exist; sudo may fail without tty, caught by subshell exit
                    curl -fsSL https://repo.charm.sh/apt/gpg.key | sudo gpg --dearmor -o /etc/apt/keyrings/charm.gpg 2>/dev/null # REASON: gpg warnings are cosmetic; failure caught by subshell &&
...
gpgkey=https://repo.charm.sh/yum/gpg.key' | sudo tee /etc/yum.repos.d/charm.repo >/dev/null
            ...
            (
                ...
                tar -xzf gum.tar.gz
                if sudo mv gum /usr/local/bin/gum 2>/dev/null; then # REASON: sudo may not be available; falls back to ~/.local/bin below
                    :
                else
                    mkdir -p ~/.local/bin
...
run_with_spinner() {
    ...
    if [[ "$GUM_AVAILABLE" == "true" && "$QUIET" -eq 0 ]]; then
        ...
        if declare -f "$1" >/dev/null 2>&1; then # REASON: testing if arg is a function; non-function is not an error
...
# ============================================================================
usage() {
    check_gum || true # REASON: gum is optional cosmetic dependency; usage falls back to ANSI
...
check_gum || true # REASON: gum is optional cosmetic dependency; installer works without it
...
do_uninstall() {
    ...
    for rc in "$HOME/.bashrc" "$HOME/.zshrc" "$HOME/.profile" "$HOME/.config/fish/config.fish"; do
        if [ -f "$rc" ] && grep -q "# br installer" "$rc" 2>/dev/null; then # REASON: grep on existing file; 2>/dev/null suppresses binary-file warnings
            if [[ "$OSTYPE" == "darwin"* ]]; then
                sed -i '' '/# br installer/d' "$rc" 2>/dev/null || true # REASON: rc file may be read-only; uninstall continues with remaining files
            else
                sed -i '/# br installer/d' "$rc" 2>/dev/null || true # REASON: rc file may be read-only; uninstall continues with remaining files
            fi
            log_step "Cleaned $rc"
...
resolve_version() {
    ...
    while [ $attempts -lt $MAX_RETRIES ] && [ -z "$tag" ]; do
        ...
        if command -v curl &>/dev/null; then
            tag=$(curl -fsSL \
                ...
                "$latest_url" 2>/dev/null | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/' || echo "") # REASON: network may fail; retried in loop and falls back to redirect/source build
        elif command -v wget &>/dev/null; then
            tag=$(wget -qO- --timeout=30 "$latest_url" 2>/dev/null | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/' || echo "") # REASON: network may fail; retried in loop and falls back to redirect/source build
        fi
    ...
    if command -v curl &>/dev/null; then
        tag=$(curl -fsSL -o /dev/null -w '%{url_effective}' "$redirect_url" 2>/dev/null | sed -E 's|.*/tag/||' || echo "") # REASON: redirect fallback; failure handled by empty-tag check below
    fi
...
acquire_lock() {
    if mkdir "$LOCK_DIR" 2>/dev/null; then # REASON: atomic lock acquire; failure means lock exists, handled below
        LOCKED=1
        echo $$ > "$LOCK_DIR/pid"
        return 0
    fi
    ...
    if [ -f "$LOCK_DIR/pid" ]; then
        local old_pid
        old_pid=$(cat "$LOCK_DIR/pid" 2>/dev/null || echo "") # REASON: pid file may vanish in race; empty string handled below

        # Check if process is still running
        if [ -n "$old_pid" ] && ! kill -0 "$old_pid" 2>/dev/null; then # REASON: kill -0 probes process existence; stderr noise expected for dead PIDs
            log_warn "Removing stale lock (PID $old_pid not running)"
            rm -rf "$LOCK_DIR"
            if mkdir "$LOCK_DIR" 2>/dev/null; then # REASON: atomic lock re-acquire after stale removal; failure means race lost
                LOCKED=1
                echo $$ > "$LOCK_DIR/pid"
                return 0
            fi
        ...
        local lock_age=0
        if [[ "$OSTYPE" == "darwin"* ]]; then
            lock_age=$(( $(date +%s) - $(stat -f %m "$LOCK_DIR/pid" 2>/dev/null || echo 0) )) # REASON: pid file may vanish in race; defaults to 0 (lock treated as fresh)
        else
            lock_age=$(( $(date +%s) - $(stat -c %Y "$LOCK_DIR/pid" 2>/dev/null || echo 0) )) # REASON: pid file may vanish in race; defaults to 0 (lock treated as fresh)
        fi

        if [ "$lock_age" -gt 300 ]; then
            log_warn "Removing stale lock (age: ${lock_age}s)"
            rm -rf "$LOCK_DIR"
            if mkdir "$LOCK_DIR" 2>/dev/null; then # REASON: atomic lock re-acquire after age-based stale removal; failure means race lost
                LOCKED=1
                echo $$ > "$LOCK_DIR/pid"
                return 0
            fi
        ...
        *)
            if [ "$EASY" -eq 1 ]; then
                ...
                for rc in "$HOME/.zshrc" "$HOME/.bashrc"; do
                    if [ -f "$rc" ] && [ -w "$rc" ]; then
                        if ! grep -qF "$DEST" "$rc" 2>/dev/null; then # REASON: grep on verified-existing file; suppresses binary-file warnings
                            echo "" >> "$rc"
                ...
                if [ -f "$fish_config" ] && [ -w "$fish_config" ]; then
                    if ! grep -qF "$DEST" "$fish_config" 2>/dev/null; then # REASON: grep on verified-existing file; suppresses binary-file warnings
                        echo "" >> "$fish_config"
...
fix_alias_conflicts() {
    ...
    for rc in "$HOME/.zshrc" "$HOME/.bashrc"; do
        if [ -f "$rc" ]; then
            ...
            if ! grep -q "unalias br.*# br installer" "$rc" 2>/dev/null; then # REASON: grep on verified-existing file; suppresses binary-file warnings
                if grep -q "alias br=" "$rc" 2>/dev/null || grep -q "\.bun" "$rc" 2>/dev/null; then # REASON: grep on verified-existing file; suppresses binary-file warnings
                    echo "" >> "$rc"
                    echo "unalias br 2>/dev/null  # br installer - remove conflicting alias" >> "$rc" # REASON: unalias in generated rc content; alias may not exist at shell init
...
install_skills() {
    ...
    for skill in "${skills[@]}"; do
        ...
        mkdir -p "$claude_skills_dir/$skill_name/references" 2>/dev/null || true # REASON: parent dir may be unwritable; skills install is best-effort
        mkdir -p "$claude_skills_dir/$skill_name/scripts" 2>/dev/null || true # REASON: parent dir may be unwritable; skills install is best-effort
        mkdir -p "$claude_skills_dir/$skill_name/subagents" 2>/dev/null || true # REASON: parent dir may be unwritable; skills install is best-effort
        mkdir -p "$codex_skills_dir/$skill_name/references" 2>/dev/null || true # REASON: codex may not be installed; skills install is best-effort
        mkdir -p "$codex_skills_dir/$skill_name/scripts" 2>/dev/null || true # REASON: codex may not be installed; skills install is best-effort
        mkdir -p "$codex_skills_dir/$skill_name/subagents" 2>/dev/null || true # REASON: codex may not be installed; skills install is best-effort
...
ensure_rust() {
    ...
    if command -v cargo >/dev/null 2>&1; then # REASON: standard command-existence check pattern
        return 0
    fi
...
prepare_for_build() {
    ...
    pkill -9 -f "cargo build" 2>/dev/null || true # REASON: no stuck cargo processes is the normal case; not an error
    # Clear cargo locks
    rm -f ~/.cargo/.package-cache 2>/dev/null || true # REASON: lock file may not exist; cleanup is best-effort
    rm -f ~/.cargo/registry/.crate-cache.lock 2>/dev/null || true # REASON: lock file may not exist; cleanup is best-effort

    # Clean up old br build directories
    rm -rf /tmp/br-build-* 2>/dev/null || true # REASON: no old builds is the normal case; not an error
    ...
    if [ "$avail_kb" -lt 1048576 ]; then
        ...
        rm -rf /tmp/cargo-target 2>/dev/null || true # REASON: dir may not exist; low-disk cleanup is best-effort
        rm -rf ~/.cargo/registry/cache 2>/dev/null || true # REASON: dir may not exist; low-disk cleanup is best-effort
    fi
...
install_binary_atomic() {
    ...
    if ! mv -f "$tmp_dest" "$dest"; then
        rm -f "$tmp_dest" 2>/dev/null || true # REASON: cleanup after failed mv; file may already be gone
...
find_binary_candidate() {
    ...
    if is_windows_binary "$binary_name"; then
        bin=$(find "$search_root" -name "$binary_name" -type f 2>/dev/null | head -1) # REASON: find may traverse unreadable dirs; permission errors are harmless noise
    else
        bin=$(find "$search_root" -name "$binary_name" -type f -perm -111 2>/dev/null | head -1) # REASON: find may traverse unreadable dirs; permission errors are harmless noise
    fi
...
download_release() {
    ...
    case "$archive_name" in
        *.tar.gz)
            if ! tar -xzf "$TMP/$archive_name" -C "$TMP" 2>/dev/null; then # REASON: tar warnings (timestamps, permissions) are noise; real failure caught by exit code
                return 1
            fi
            ;;
        *.zip)
            if command -v unzip &>/dev/null; then
                if ! unzip -q "$TMP/$archive_name" -d "$TMP" 2>/dev/null; then # REASON: unzip warnings are noise; real failure caught by exit code
                    return 1
                fi
            elif command -v bsdtar &>/dev/null; then
                if ! bsdtar -xf "$TMP/$archive_name" -C "$TMP" 2>/dev/null; then # REASON: bsdtar warnings are noise; real failure caught by exit code
                    return 1
                fi
...
check_conflicts() {
    ...
    if [ ${#conflicts[@]} -gt 0 ]; then
        ...
        # Check PATH priority
        local active_br
        active_br=$(command -v br 2>/dev/null || echo "") # REASON: br may not be in PATH yet; empty string handled below
...
print_summary() {
    ...
    installed_version=$("$DEST/$BINARY_NAME" --version 2>/dev/null || echo "unknown") # REASON: binary may not be in PATH yet; defaults to "unknown" for display
...
main() {
    ...
    if [ "$VERIFY" -eq 1 ]; then
        ...
        "$DEST/$BINARY_NAME" --version || true # REASON: self-test; binary may fail on first run due to missing config but install succeeded
